    ...
    while (!stopRequested()) {
        ...
        try {
            ...
            const float remaining_time =
                step_settings.steplen - 1e-6f * (porting::getTimeUs() - t0);
...
Server::Server(const std::string &path_world, const SubgameSpec &gamespec,
               ...
               ChatInterface *iface, std::string *shutdown_errmsg)
    : m_bind_addr(bind_addr), m_path_world(path_world), m_gamespec(gamespec),
      ...
      m_clients(m_con), m_admin_chat(iface), m_shutdown_errmsg(shutdown_errmsg),
...
Server::~Server() {
    ...
    if (m_env) {
        ...
        kickAllPlayers(SERVER_ACCESSDENIED_SHUTDOWN, kick_msg, reconnect);
...
void Server::init() {
    ...
    try {
        ...
        throw ServerError(std::string("Failed to initialize world: ") +
                          e.what());
    }
    ...
    // Must be created before mod loading because we have some inventory
    // creation
    ...
    for (const std::string &path : paths) {
        TextureOverrideSource override_source(path + DIR_DELIM +
                                              "override.txt");
        m_nodedef->applyTextureOverrides(
            override_source.getNodeTileOverrides());
    ...
    m_csm_restriction_noderange =
        g_settings->getU32("csm_restriction_noderange");
}
...
void Server::AsyncRunStep(float dtime, bool initial_step) {
    ...
    */
    ...
    if (m_map_timer_and_unload_interval.step(dtime,
        ...
        m_env->getMap().timerUpdate(
            map_timer_and_unload_dtime,
            std::max(g_settings->getFloat("server_unload_unused_data_timeout"),
                     0.0f),
            -1);
    }
    ...
    */
    {
        ...
        // This guarantees that each object recomputes its cache only once per
        // server step, unless get_effective_observers is called. If we were to
        // update observer sets eagerly in set_observers instead, the total
        // costs of calls to set_observers could theoretically be higher.
...

        {
            ...
            for (const auto &client_it : clients) {
                ...
                for (const auto &buffered_message : buffered_messages) {
                    // If object does not exist or is not known by client, skip
                    // it
                    ...
                    for (const ActiveObjectMessage &aom : *list) {
                        // Send position updates to players who do not see the
                        // attachment
                        ...
                        std::string &buffer =
                            aom.reliable ? reliable_data : unreliable_data;
                        char idbuf[2];
    ...
    */
    ...
    /*
        ...
        Doing this every 2s is left over from old code, unclear if this is still
       needed.
    */
    {
...
void Server::yieldToOtherThreads(float dtime) {
     ...
     * While the emerge thread needs it just once or twice for every processed
     * item the server thread uses it much more generously. This is usually not
     * a problem as the server sleeps between steps, which leaves enough chance.
     * But if the server is overloaded it's busy all the time and
     ...
     * ever would.
     *
     * Workaround: If we detect that the server is overloaded, introduce some
     * careful artificial sleeps to leave the emerge threads enough chance to do
     * their job.
     *
     * In the future the emerge code should be reworked to exclusively use a
     * result queue, thereby avoiding this problem (and terrible workaround).
     */
    ...
    // put an upper limit to not cause too much lag, also so this doesn't become
    // self-sustaining
...
PlayerSAO *Server::StageTwoClientInit(session_t peer_id) {
    ...
    std::unique_ptr<PlayerSAO> sao;
    {
        ...
        if (client) {
            ...
            sao = emergePlayer(playername.c_str(), peer_id,
    ...
    */
    {
        NetworkPacket notice_pkt(TOCLIENT_UPDATE_PLAYER_LIST, 0,
                                 PEER_ID_INEXISTENT);
    ...
    {
        std::string ip_str =
            getPeerAddress(player->getPeerId()).serializeString();
...
inline void Server::handleCommand(NetworkPacket *pkt) {
    const ToServerCommandHandler &opHandle =
        toServerCommandTable[pkt->getCommand()];
...
void Server::ProcessData(NetworkPacket *pkt) {
    ...
    try {
        ...
        if (toServerCommandTable[command].state ==
        ...
        u8 peer_ser_ver =
            getClient(peer_id, CS_InitDone)->serialization_version;
...
void Server::HandlePlayerHPChange(PlayerSAO *playersao,
...
void Server::SendItemDef(session_t peer_id, IItemDefManager *itemdef,
...
void Server::SendNodeDef(session_t peer_id, const NodeDefManager *nodedef,
                         u16 protocol_version) {
...
void Server::SendAddParticleSpawner(session_t peer_id, u16 protocol_version,
                                    ...
                                    u16 attached_id, u32 id) {
    static thread_local const float radius =
        g_settings->getS16("max_block_send_distance") * MAP_BLOCKSIZE * BS;

    if (peer_id == PEER_ID_INEXISTENT) {
        std::vector<session_t> clients = m_clients.getClientIDs();
        const v3f pos = (p.pos.start.min.val + p.pos.start.max.val +
                         p.pos.end.min.val + p.pos.end.max.val) /
                        4.0f * BS;
        const float radius_sq = radius * radius;
        ...
        const bool distance_check =
            !attached_id && p.time <= 1.0f && p.time != 0.0f;
    ...
    { // serialize new fields
        ...
        if (protocol_version < 42) {
            ...
            pkt << p.pos.start.bias << p.vel.start.bias << p.acc.start.bias
                << p.exptime.start.bias << p.size.start.bias;
...
void Server::SendHUDSetParam(session_t peer_id, u16 param,
...
void Server::SendMovePlayer(PlayerSAO *sao) {
    ...
    NetworkPacket pkt(TOCLIENT_MOVE_PLAYER, sizeof(v3f) + sizeof(f32) * 2,
                      sao->getPeerID());
    pkt << sao->getBasePosition() << sao->getLookPitch()
        << sao->getRotation().Y;
...
void Server::SendActiveObjectRemoveAdd(RemoteClient *client,
    ...
    NetworkPacket pkt(TOCLIENT_ACTIVE_OBJECT_REMOVE_ADD,
                      2 * removed_objects.size() + 32 * added_objects.size(),
                      client->peer_id);
    ...
    // Note: Do yet NOT stop or remove object-attached sounds where the object
    // goes out of range (client side). Such sounds would need to be re-sent
    // when coming into range. Currently, the client will initiate
    // m_playing_sounds clean ups indirectly by
...
void Server::SendActiveObjectMessages(session_t peer_id,
    ...
    m_clients.sendCustom(pkt.getPeerId(), reliable ? ccf.channel : 1, &pkt,
                         reliable);
}
    ...
    do {
        ...
        if (free_id == m_playing_sounds_id_last_used) {
            return 0;
             ...
             m_playing_sounds.find(free_id) != m_playing_sounds.end());
...
s32 Server::playSound(ServerPlayingSound &params, bool ephemeral) {
    ...
    pkt << id << params.spec.name << gain << (u8)params.type << pos
        << params.object << params.spec.loop << params.spec.fade
        << params.spec.pitch << ephemeral << params.spec.start_time;
...
void Server::sendAddNode(v3s16 p, MapNode n,
                         std::unordered_set<u16> *far_players,
...
void Server::sendNodeChangePkt(NetworkPacket &pkt, v3s16 block_pos, v3f p,
                               float far_d_nodes,
                               std::unordered_set<u16> *far_players) {
    ...
    for (session_t client_id : clients) {
        ...
        if (!client->isBlockSent(block_pos) ||
            (sao && sao->getBasePosition().getDistanceFrom(p) > maxd)) {
            if (far_players) {
...
void Server::sendMetadataChanged(const std::unordered_set<v3s16> &positions,
    ...
    for (session_t i : clients) {
        ...
        os.str("");
        meta_updates_list.serialize(os, client->serialization_version, false,
                                    true, true);
...
bool Server::addMediaFile(const std::string &filename,
    ...
    const char *supported_ext[] = {".png", ".jpg", ".tga", ".ogg", ".x", ".b3d",
                                   ".obj", ".gltf", ".glb",
...
void Server::sendMediaAnnouncement(session_t peer_id,
    ...
    for (size_t i = 0; i < 3; i++) {
        lang_suffixes[i].append(".").append(lang_code).append(
            translation_formats[i]);
    }
    ...
    auto include = [&](const std::string &name, const MediaInfo &info) -> bool {
        ...
        for (size_t j = 0; j < 3; j++) {
            if (str_ends_with(name, translation_formats[j]) &&
                !str_ends_with(name, lang_suffixes[j])) {
                return false;
            }
        }
        return true;
...
void Server::stepPendingDynMediaCallbacks(float dtime) {
    ...
    for (auto it = m_pending_dyn_media.begin();
         it != m_pending_dyn_media.end();) {
        it->second.expiry_timer -= dtime;
        bool del =
            it->second.waiting_players.empty() || it->second.expiry_timer < 0;
        ...
        if (!name.empty()) {
            ...
            // if no_announce isn't set we're definitely deleting the wrong
            // file!
...
void Server::SendMinimapModes(session_t peer_id,
    ...
    for (auto &mode : modes) {
        pkt << (u16)mode.type << mode.label << mode.size << mode.texture
            << mode.scale;
    }
...
void Server::sendDetachedInventory(Inventory *inventory,
                                   const std::string &name, session_t peer_id) {
...
void Server::UpdateCrafting(RemotePlayer *player) {
    ...
    getCraftingResult(&player->inventory, preview, output_replacements, false,
                      this);
...
std::wstring Server::handleChat(const std::string &name, std::wstring wmessage,
    ...
    // Line to send
    std::wstring line;
    // Whether to send line to the player that sent the message, or to all
    // players
...
bool Server::hudChange(RemotePlayer *player, u32 id, HudElementStat stat,
                       void *data) {
    if (!player) {
        return false;
    }
...
Address Server::getPeerAddress(session_t peer_id) {
    // Note that this is only set after Init was received in
    // Server::handleCommand_Init
...
void Server::setLocalPlayerAnimations(RemotePlayer *player,
    ...
    SendLocalPlayerAnimations(player->getPeerId(), animation_frames,
                              frame_speed);
}

void Server::setPlayerEyeOffset(RemotePlayer *player, v3f first, v3f third,
                                v3f third_front) {
    sanity_check(player);
    if (std::tie(player->eye_offset_first, player->eye_offset_third,
                 player->eye_offset_third_front) ==
        std::tie(first, third, third_front)) {
        return; // no change
    }
...
bool Server::dynamicAddMedia(const DynamicMediaArgs &a) {
    ...
    if (a.filepath) {
        ...
        // Write the file to disk. addMediaFile() will read it right back but
        // this is the best way without turning the media loading code into
        // spaghetti.
    ...
    if (a.ephemeral) {
        if (!a.data) {
            ...
            // requirement that mods keep the file accessible at the original
            // path.
    ...
    if (m_env) {
        ...
        for (auto &pair : m_clients.getClientList()) {
            if (pair.second->getState() == CS_DefinitionsSent && !a.ephemeral) {
                /*
                    If a client is in the DefinitionsSent state it is too late
                   to transfer the file via sendMediaAnnouncement() but at the
                   same time the client cannot accept a media push yet. Short of
                   artificially delaying the joining process there is no way for
                   the server to resolve this so we (currently) opt not to.
                */
                warningstream << "The media \"" << filename
                              << "\" (dynamic) could "
                                 "not be delivered to "
                              << pair.second->getName()
            ...
            if (proto_ver < 40) {
                ...
                /*
                   ...
                   channel. Since the very next packet could be one that uses
                   the media, we have to push the media over ALL channels to
                   ensure it is processed before it is used. In practice this
                   means channels 1 and 0.
                */
    ...
    // Run callback for players that already had the file delivered
    // (legacy-only)
    for (session_t peer_id : delivered) {
        if (auto player = m_env->getPlayer(peer_id)) {
            getScriptIface()->on_dynamic_media_added(a.token,
                                                     player->getName());
        }
    }
...
// IGameDef interface
// Under envlock
IItemDefManager *Server::getItemDefManager() { return m_itemdef; }

const NodeDefManager *Server::getNodeDefManager() { return m_nodedef; }

ICraftDefManager *Server::getCraftDefManager() { return m_craftdef; }
...
NodeDefManager *Server::getWritableNodeDefManager() { return m_nodedef; }
...
v3f Server::findSpawnPos() {
    ...
    if (g_settings->getV3FNoEx("static_spawnpoint", staticSpawnPoint) &&
        staticSpawnPoint.has_value()) {
    ...
    for (s32 i = 0; i < 4000 && !is_good; i++) {
        ...
        v2s16 nodepos2d = v2s16(-range + myrand_range(0, range * 2),
            ...
            if (m_nodedef->get(c).drawtype == NDT_AIRLIKE ||
                ...
                if (air_count >= 2) {
                    ...
                    if (objectpos_over_limit(nodeposf)) {
                        // Exit this loop, positions above are probably over
                        // limit
                        break;
                    }
...
void Server::requestShutdown(const std::string &msg, bool reconnect,
                             float delay) {
    if (delay == 0.0f) {
...
void dedicated_server_loop(Server &server, bool &kill) {
    ...
    const float profiler_print_interval =
        g_settings->getFloat("profiler_print_interval");

    server.setStepSettings(Server::StepSettings{
        g_settings->getFloat("dedicated_server_step"), false});
...
bool Server::joinModChannel(const std::string &channel) {
    return m_modchannel_mgr->joinChannel(channel, PEER_ID_SERVER) &&
           m_modchannel_mgr->setChannelState(channel,
...
void Server::broadcastModChannelMessage(const std::string &channel,
    ...
    NetworkPacket resp_pkt(TOCLIENT_MODCHANNEL_MSG, 2 + channel.size() + 2 +
                                                        sender.size() + 2 +
                                                        message.size());
...
ModStorageDatabase *
Server::openModStorageDatabase(const std::string &world_path) {
    ...
    std::string backend = world_mt.exists("mod_storage_backend")
                              ? world_mt.get("mod_storage_backend")
                              : "files";
    if (backend == "files") {
        warningstream
...
ModStorageDatabase *
Server::openModStorageDatabase(const std::string &backend,
    ...
    throw BaseException("Mod storage database backend " + backend +
                        " not supported");
}
...
u16 Server::getProtocolVersionMin() {
    ...
    return rangelim(min_proto, SERVER_PROTOCOL_VERSION_MIN,
                    LATEST_PROTOCOL_VERSION);
}

u16 Server::getProtocolVersionMax() { return LATEST_PROTOCOL_VERSION; }
